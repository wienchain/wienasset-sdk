...
const debug = require('debug')('wienassets-sdk');
const HDWallet = require('hdwallet');
const WienAssetRpc = require('coloredcoins-rpc');
const BlockExplorerRpc = require('blockexplorer-rpc');
const bitcoin = require('bitcoinjs-lib');
const WienAssetBuilder = require('wienasset-protocol').TransactionBuilder;
...
var mainnetWienAssetHost = 'https://explorer.wienchain.com/api'
var testnetWienAssetHost = 'https://explorer-testnet.wienchain.com/api'

var mainnetBlockExplorerHost = 'https://explorer.wienchain.com/api'
var testnetBlockExplorerHost = 'https://explorer-testnet.wienchain.com/api'

var metadataServerHost = 'https://asset.wienchain.com/metadata'

var verifierPath = ''

var WienAsset = function (settings) {
  var self = this
  settings = settings || {}
  if (settings.network === 'testnet') {
    settings.wienAssetHost = settings.wienAssetHost || testnetWienAssetHost
    settings.blockExplorerHost = settings.blockExplorerHost || testnetBlockExplorerHost
  } else {
    settings.wienAssetHost = settings.wienAssetHost || mainnetWienAssetHost
    settings.blockExplorerHost = settings.blockExplorerHost || mainnetBlockExplorerHost
  }
  self.cc = new WienAssetRpc(settings.wienAssetHost)
  self.ccb = new WienAssetBuilder({ network: settings.network })
...
util.inherits(WienAsset, events.EventEmitter)

WienAsset.encryptPrivateKey = HDWallet.encryptPrivateKey
WienAsset.decryptPrivateKey = HDWallet.decryptPrivateKey
WienAsset.createNewKey = HDWallet.createNewKey
WienAsset.generateMnemonic = HDWallet.generateMnemonic
WienAsset.validateMnemonic = HDWallet.validateMnemonic

WienAsset.prototype.init = function (cb) {
  var self = this
...
WienAsset.prototype.buildTransaction = function (type, ccArgs, callback) {
  var self = this
  ...
  self.metadataServer.upload(ccArgs, function(err, ccArgs) {
    ...
    try {
      if (type === 'send') {
        ...
        tx = self.ccb.buildSendNativeTransaction(ccArgs)
      } else {
...
WienAsset.prototype.signAndTransmit = function (assetInfo, callback) {
  var self = this
  async.waterfall([
    function (cb) {
...
WienAsset.prototype.sign = function (txHex, callback) {
  this.hdwallet.sign(txHex, callback)
}

WienAsset.prototype.transmit = function (signedTxHex, callback) {
  this.chainAdapter.transmit(signedTxHex, callback)
}

WienAsset.prototype.issueAsset = function (args, callback) {
  var self = this
...
WienAsset.prototype.buildSendAssetTX = function (args, callback) {
  var self = this
  var transmit = args.transmit !== false
  async.waterfall([
    function (cb) {
...
WienAsset.prototype.buildSendWienTX = function (args, callback) {
  var self = this
  async.waterfall([
    function (cb) {
      if (args.from && Array.isArray(args.from) && args.from.length) {
    ...
    function (cb) {
      self.buildTransaction('native', args, cb)
    },
    function (assetInfo, cb) {
      if (!args.privateKey) {
        return cb(null, {unsignedTX: assetInfo.txHex});
...
WienAsset.prototype.burnAsset = function (args, callback) {
  var self = this
  var transmit = args.transmit !== false
  args.transfer = args.transfer || []
...
WienAsset.prototype.getUtxos = function (callback) {
  var self = this
  self.hdwallet.getAddresses(function(err, addresses) {
    if (err) {
      callback(err)
...
WienAsset.prototype._getUtxosForAddresses = function (addresses, callback) {
  this.chainAdapter.getAddressesUtxos(addresses, callback)
}

WienAsset.prototype.getAssets = function (callback) {
...
WienAsset.prototype.getTransactions = function (addresses, callback) {
  var self = this
...
WienAsset.prototype.getTransactionsFromAddresses = function (addresses, callback) {
...
WienAsset.prototype.getAssetMetadata = function (assetId, utxo, full, callback) {
  var self = this
...
WienAsset.prototype.on = function (eventKey, callback) {
  switch (eventKey) {
    case 'newTransaction':
...
WienAsset.prototype.onNewCCTransaction = function (callback) {
  var self = this
...
WienAsset.prototype.onProgress = function(callback) {
...
WienAsset.prototype.getIssuedAssetsFromTransactions = function (addresses, transactions) {
  var issuances = []
...
WienAsset.prototype.getIssuedAssets = function (transactions, callback) {
  var self = this
...
WienAsset.prototype.getAddressInfo = function (address, cb) {
...
WienAsset.prototype.getStakeHolders = function (assetId, numConfirmations, cb) {
...
WienAsset.prototype.verifyIssuer = function (assetId, json, cb) {
